...
for (const mode of ["offline", "cloud"] as const) {
  test(`@inputs all persisted inputs save in ${mode} mode`, async ({ page }) => {
    ...
    await cloudPanel.locator(".cloud-mode-field select").selectOption(mode);
    if (mode === "offline") {
      await expect(page.locator("header .sync-pill")).toContainText("Local");
    } else {
      await expect(page.locator("header .sync-pill")).toContainText("Cloud (not synced)");
    }
    ...
    if (mode === "offline") {
      await expect(page.locator("header .sync-pill")).toContainText("מקומי");
    } else {
      await expect(page.locator("header .sync-pill")).toContainText("לא מסונכרן");
    }
    ...
    expect(persisted.localStorage.cloudMode).toBe(mode);
